#! perl  
####################################################################
# v.8		8/20/17	Sasha - single line output;
#					   yaml config updated to v.6.1
...
if ($config->{diagnostics}->{OnlyOutputDiagnostics} eq "1") {$OnlyOutputDiagnostics=1}
...
$SNP	=$config->{genes}->{$gene_name}->{SNP};
	...
	#~ if($SNP =~ m/(\w+)\[\w+\/\w+\](\\\w+)/gi)
		{	$SNPleftFlank=$1; 
			$SNPrightFlank=$2;
			if($SNPrightFlank eq "_"){$SNPrightFlank="\.";} ### empty
		}
...
### ~~~ DEAL with short R1
	...
	else{ print STDERR "$0: No Supp flag set:	r1=$r1{FLAG}   r2=$r2{FLAG}\n"};
...
# loop through and print output
	...
	if (exists($data{$samples_barcodes[$i][1]})){
		...
		$output_line.="\t0\t0\t0\t0\t0\t{NA}\t<NA>";
		$bc_mismatch++;}

	### now print the assembled line
	unless ($OnlyOutputDiagnostics){print "$output_line\n";}
	}
}	
##################
#~ if($TryRevcomp){
...
#~ my $header="Gene/File	AlignedPairs	UnpairedReads	MAPQ<$MAPQmin	(%)	Tail2missing	(NA%)	BCdeclared	BCnotFound	ReadsUnknownBC (<-%)	AllPassed	SNPnotFound (%)	ExtraUMIs (%)\n";
	my $header="#	From	ReadPairs	UnpairedReads	MAPQ<12	(% of $read_pairs)	Tail2_notFound	(NA%)	BCdeclared	BCnotFound	ReadsUnknownBC	(% of $read_pairs)	AllPassed	(% of $read_pairs)	UniqueGTCalls	(% all passed)	SNPnotFound	(% all passed)	ExtraUMIs	(% all passed)\n";
	if($all_genotyped == 0){$all_genotyped=-1};
	if($read_pairs == 0){$read_pairs=-1};
	if($accum == 0){$accum=-1};
	...
	$info=("#	$infile	$read_pairs	$QNAME_mismatch	$MAPQ_low	");
	$tmp=sprintf("%.1f", 100*$MAPQ_low/$read_pairs); 
	$info.="$tmp%	";
	$info.="$T2_not_found	NA	";
		$sb=$#samples_barcodes+1;	
	$info.="$sb	$bc_mismatch	";
		$tmp=sprintf("%.1f", 100*$unknown_bc/$read_pairs); 
	$info.="$unknown_bc	$tmp%	";			
	
	#~ $tmp=sprintf("%.1f	", 100*$MAPQ_low/$read_pairs); $info.=$tmp;
		$tmp=sprintf("%.1f", 100*$all_processed/$read_pairs);	
	$info.="$all_processed	$tmp%	";
	
		$sb1=$sb-$bc_mismatch; 		
		$tmp=sprintf("%.1f", 100*$accum/$all_processed); 
	$info.="$accum	$tmp%	";
		$tmp=sprintf("%.1f", 100*$flank_not_found/$accum);
	$info.="$flank_not_found	$tmp%	";
		$tmp=sprintf("%.1f", 100*$extraUMIaccum/$all_processed);
	$info.="$extraUMIaccum	$tmp%\n";
	
#~ Aligned Reads
#~ MAPQ<12
#~ % reads with MAPQ<12
#~ T2 not found
#~ % reads with T2 not found
#~ Reads with undeclared BCs
#~ % reads with undeclared BCs
#~ Reads that pass all filters
#~ Unique genotype calls
#~ %unique UMI
#~ SNP flank not found
#~ %SNP flank not found
#~ extra UMIs

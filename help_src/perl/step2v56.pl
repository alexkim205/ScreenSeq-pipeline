#						sped up sort
# v.5.6	07/26/17	Sasha	housekeeping; updated to 6.1 YAML
...
$SNPUMI_SCRIPT=$config->{scripts}->{SNPUMI_script};
...
elsif	(($isSNPexpt eq 'yes')&&($isUMIexpt eq 'yes')) {$SCRIPT=$SNPUMI_SCRIPT} 
...
unless (	defined($sam) &&
		...
		defined($isUMIexpt) &&
		defined($isSNPexpt)
		) {die "$0: ERROR: some variables not defined in YAML file $yaml\npossible syntax issue\n"};
